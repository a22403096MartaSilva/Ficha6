...
for language in ['PT', 'ENG']:
    ...
    with open(os.path.join('data',f"ULHT{course}-{language}.json"), "w", encoding="utf-8") as f:
    ...
    for uc in response_dict['courseFlatPlan']:
      ...
      with open(os.path.join('data',f"{uc['curricularIUnitReadableCode']}-{language}.json"), "w", encoding="utf-8") as f:
